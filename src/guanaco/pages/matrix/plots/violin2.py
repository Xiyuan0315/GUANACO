...
DEFAULT_COLORS = list(GUANACO_QUALITATIVE)
...
def _linear_model_p_values(df, meta1, meta2, interaction=False):
    ...
    try:
        ...
        model = ols(formula, data=df_model).fit()
        p_values["model_summary"] = {
            "meta1_p": model.pvalues[
                f"C({meta1})[T.{df_model[meta1].cat.categories[1]}]"
            ],
            "meta2_p": model.pvalues[
                f"C({meta2})[T.{df_model[meta2].cat.categories[1]}]"
            ]
            if len(df_model[meta2].unique()) > 1
            else np.nan,
            "r_squared": model.rsquared,
            "aic": model.aic,
        }
...
def _pseudobulk_p_values(df, meta1, meta2):
    ...
    if len(groups) == 2:
        return {
            "overall": _safe_stat_test(
                ttest_ind, groups[0], groups[1], equal_var=False
            ),
    ...
    try:
        ...
        try:
            with warnings.catch_warnings():
                ...
                model = mixedlm(formula, data=df_model, groups=df_model[meta2]).fit(
                    reml=False
                )
...
def calculate_p_values_by_mode(
...
def _build_expression_frame(
    ...
    if transformation:
        expression_data = apply_transformation(
            expression_data, transformation, copy=False
        )
...
def _resolve_color_map(
    design_type, meta1_cats, meta2_cats, color_map=None, palette=None
):
...
def _add_nested_mode2_annotations(
    fig, p_values, df, meta1, meta2, meta1_groups, y_max, p_label
):
...
def _add_crossed_mode2_annotations(
    fig, p_values, df, meta1_groups, meta2, y_max, p_label
):
    meta2_groups = sorted(df[meta2].unique())
    center_m2 = meta2_groups[len(meta2_groups) // 2]
    for facet, p_val in p_values.items():
        if _is_valid_facet_p_value(facet, p_val, meta1_groups):
            _add_data_annotation(
                fig, f"{facet}_{center_m2}", y_max, _sig_stacked(p_val, p_label)
            )


def _add_grouped_mode2_annotations(
    fig, p_values, df, meta1_groups, meta2, y_max, p_label
):
    meta2_groups = sorted(df[meta2].unique())
    ...
    for facet, p_val in p_values.items():
        if _is_valid_facet_p_value(facet, p_val, meta1_groups):
            ...
            _add_data_annotation(
                fig, center_pos, y_max, _sig_stacked(p_val, p_label), xref="x"
            )


def _add_mode2_annotations(
    fig, p_values, df, meta1, meta2, split_violin, design_type, y_max, p_label
):
    meta1_groups = sorted(df[meta1].unique())

    if design_type == "nested":
        _add_nested_mode2_annotations(
            fig, p_values, df, meta1, meta2, meta1_groups, y_max, p_label
        )
    elif split_violin:
        _add_split_mode2_annotations(fig, p_values, meta1_groups, y_max, p_label)
    elif design_type in ["crossed", "sparse"]:
        _add_crossed_mode2_annotations(
            fig, p_values, df, meta1_groups, meta2, y_max, p_label
        )
    else:
        _add_grouped_mode2_annotations(
            fig, p_values, df, meta1_groups, meta2, y_max, p_label
        )
...
def _add_model_annotations(fig, p_values, mode, meta1, meta2):
    if "model_summary" in p_values:
        _add_paper_annotation(
            fig, _model_summary_text(p_values["model_summary"], mode, meta1, meta2)
        )
    elif "overall" in p_values:
        _add_paper_annotation(fig, _sig_inline(p_values["overall"], "p"))


def add_p_value_annotations_new(
    fig,
    p_values,
    df,
    mode,
...
def plot_violin2_new(
    ...
    design_type = (
        _detect_design_pattern(df, meta1, meta2)
        if meta2 and mode != "mode1"
        else "single"
    )

    color_map = _resolve_color_map(
        design_type, meta1_cats, meta2_cats, color_map, palette
    )
    ...
    shared_bandwidth = resolve_violin_bandwidth(df["Expression"], bandwidth)
    if shared_bandwidth is not None:
        for trace in fig.data:
            if getattr(trace, "type", None) == "violin":
                trace.bandwidth = shared_bandwidth
    ...
    _apply_common_layout(
        fig, df, key, title, xaxis_title, meta2, mode, test_description
    )
...
def _create_single_meta_violin(
    fig, df, key, meta1, meta1_cats, show_box, color_map=None, show_points=False
):
        ...
        for j, m2_group in enumerate(meta2_cats):
            ...
            fig.add_trace(
                go.Violin(
                    ...
                    **_violin_style(
                        color_map.get(
                            m2_group, DEFAULT_COLORS[j % len(DEFAULT_COLORS)]
                        ),
                        show_box,
                        show_points,
                    ),
...
def _create_nested_violin(
    ...
    sorted_meta2 = sorted(
        meta2_cats,
        key=lambda x: (
            meta1_cats.index(meta2_to_meta1.get(x, meta1_cats[0]))
            if meta2_to_meta1.get(x) in meta1_cats
            else 999,
            str(x),
        ),
    )
    ...
    for m2_group in sorted_meta2:
        ...
        fig.add_trace(
            go.Violin(
                ...
                **_violin_style(
                    color_map.get(
                        m1_group, DEFAULT_COLORS[m1_idx % len(DEFAULT_COLORS)]
                    ),
                    show_box,
                    show_points,
                ),
    ...
    fig.update_layout(violingap=0.1)
...
def _apply_common_layout(
    ...
    fig.update_layout(
        ...
        legend=dict(
            font=dict(size=12),
            title=dict(
                text=meta2 if mode != "mode1" and meta2 else "", font=dict(size=12)
            ),
            tracegroupgap=5,
        ),
